...
#cluster 55 hardcoded
pure_cluster_sno = np.append(np.where(db.labels_== 55)[0],pure_cluster_sno).astype(int)
l = np.where(db.labels_ == 55)[0][0]
temp = []
for j in X_train[l].nonzero()[1]:
        temp.append(symbols[j-1])

with open('Composites/spam/55.txt' ,'a') as file:
    for i in range(73):
        file.write(' '.join(temp) + '\n')
#now we can prepare composites for svm here also but its better to prepare a libsvm type file through shell .
